...
; t represent time
; u represent
; this code takes this veriables and apply formula and returns Ut
...
; as a result code returns with positive root of the equation with larger-root method.  
...
; YOUR DOCUMENTATION HERE...  	    	  	  
;  	    	  	  
;(define hit?  	    	  ; Not so efficent Way 	  
;(lambda (angle velocity elevation burst-radius target-distance)
;(and
;(if (>= (+ (distance angle velocity elevation) (+ burst-radius 0.1)) target-distance) #t #f)
;(if (<= (- (distance angle velocity elevation) (- burst-radius 0.1)) target-distance) #t #f)
;)))

(define hit?  	    	  	  
  (lambda (angle velocity elevation burst-radius target-distance) (if ( >= (abs (+ ( - (distance angle velocity elevation) target-distance ))) (+ 0.0000001 burst-radius)) #f  #t)))
...
; It has a epsilon value which is error preventer ! I added into 0.000001
...
;  	    	  	  
; YOUR TEST CASES HERE...  	    	  	  
;(hit? 30 45 0 1 179)	; -> #f target distance 10+Burst - distance 179
;(distance 30 45 0)     ; -> 179
;(hit? 25 30 0 1 10)	; -> #f target distance 10+Burst - distance 45
;(distance 25 30 0)     ; -- 45
;(hit? 10 10 10 1 10)	; -> #f target distance 10+Burst - distance 16
...
(define find-velocity-helper  	    	  	  
  (lambda (angle min-vel max-vel elevation burst-radius target-distance)
    ...
    (cond
      ...
      (else  ( if (short? angle  (/ (+ max-vel min-vel) 2)  elevation target-distance   )
...
; Order of the growth of my procedure is (log n) since it's include binary search !
...
(define find-throw-helper  	    	  	  
  (lambda (angle elevation burst-radius target-distance)
    ...
    (cond
      ...
      ((= target-distance 0) (tell-gorilla 0 0))
      ...
      (else  (tell-gorilla angle (find-velocity angle elevation burst-radius target-distance) )  ) ; it breaks loop ;Base Case
...
;  	    	  	  
; YOUR TEST CASES HERE...  	    	  	  
;(find-throw 10 1   0)   ; -> (0,0) 	    	  	  
;(find-throw 20 0  50)   ; -> epsilon value was added into hit? function, for covering up error epsilon value was added near to burst radius (+ burst-radius 0.000001) 	    	  	  
;(find-throw 0  0  50)   ; -> epsilon value was added into hit? function, for covering up error epsilon value was added near to burst radius (+ burst-radius 0.000001)
;(find-throw 50 0  50)   ; -> epsilon value was added into hit? function, for covering up error epsilon value was added near to burst radius (+ burst-radius 0.000001)
;(find-throw 50 0   0)   ; -> When target distance 0 it automacally returns (0,0) bundle 
;(find-throw 0 50   0)   ; -> When target distance 0 it automacally returns (0,0) bundle 
;(find-throw 0  0   0)	 ; -> When target distance 0 it automacally returns (0,0) bundle 


; What is the order of growth in time of your procedure? order of growth in time is n, since it's include linear search !. 
...
;check short-bundle?
;This code checks the throwing with bundle is hit front of the target or not, if it hits front of the target returns #t otherwise returns #f

(define short-bundle?  	    	  	  
  (lambda (num angle velocity elevation target-distance)
    (if (< (bundle-distance num angle velocity elevation) target-distance) 	    	  	  
        #t #f)))

;check long-bundle
;This code checks the throwing with bundle is hit far from the target or not, if it hits far from the target returns #t otherwise returns #f
...
;min-vel and max-vel was set as same as the find-velocity 

(define *min-velocity-bundle* 0 )  	    	  	  
(define *max-velocity-bundle* 50)  	    	  	  

;bundle-velocity
;this code calls the bundle-velocity-helper 

(define bundle-velocity  	    	  	  
  (lambda (num angle elevation burst-radius target-distance)
    (bundle-velocity-helper num angle *min-velocity-bundle* *max-velocity-bundle*
                            elevation burst-radius target-distance)))

;bundle-velocity-helper
;this code works recursively, it works same as find-velocity. 


(define bundle-velocity-helper  	    	  	  
  (lambda (num angle min-vel max-vel elevation burst-radius target-distance)
    ;; idea is to try two extremes, if out of range return #f
    ;; else use binary search  	    	  	  
    (cond
      
      ((short-bundle? num angle max-vel elevation target-distance ) #f)
      
      ((long-bundle? num angle min-vel elevation target-distance  ) #f)
      
      ((hit-bundle? num angle (/ (+ max-vel min-vel) 2) elevation burst-radius target-distance )  (* .5 (+ min-vel max-vel)))	 
      
      (else  ( if (short-bundle? num angle  (/ (+ max-vel min-vel) 2)  elevation target-distance   )
                  
                  
                  (bundle-velocity-helper num angle
                                          (/ (+ max-vel min-vel) 2)
                                          max-vel
                                          elevation
                                          burst-radius
...
;hit-bundle?
;it checks, bundle-throw, hit or not hit the target with burst radius. This fucntion  is boolean function, returns #f or #t 

(define hit-bundle?  	    	  	  
  (lambda (num angle velocity elevation burst-radius target-distance) (if ( >= (abs (+ ( - (bundle-distance num angle velocity elevation) target-distance ))) (+ 0.0001 burst-radius)) #f  #t)))

;find-bundle-throw-helper
; it searches angle from 0 to 90 if it founds fit velocity returns angle and fit velocity as same as find-throw 
...
(define find-bundle-throw-helper  	    	  	  
  (lambda (num angle elevation burst-radius target-distance)
    ...
    (cond
      ...
      ((equal? #f (bundle-velocity num angle elevation burst-radius target-distance)) (find-bundle-throw-helper num (+ angle 0.1) elevation burst-radius target-distance)) ; recursive trigger
      
      (else  (tell-gorilla angle (bundle-velocity num angle elevation burst-radius target-distance) )  ) ; it breaks loop ;Base Case
      
      )
    ))

;main method;
;this is the main method, and takes find-bundle-throw-helper and set start angle of find-bundle-throw-helper 0 
...
;(trace find-bundle-throw-helper)
;(trace bundle-velocity)
;(trace bundle-velocity-helper)
...
;(find-bundle-throw 10 50 default-radius 400)   ; -> (73 9) ---> It returns (10 11.13) --> When angle set 73 its works, when starts with 0 reusult = (0.4 45.9)
;  	    	  	  
; YOUR TEST CASES HERE...  	    	  	  
;(find-bundle-throw 10 50 0 400)         ; ->  (0.4 45.9)
;(find-bundle-throw 4 50 0 400)          ; ->  (0.4 45.88) error also added for this case, when burst-radius = "0" its works 
;(find-bundle-throw 100 10000 30 5000)   ; ->  (1.4 48.8)	    	  	  
;(find-bundle-throw 1000 0 40 6000000)   ; ->  (2.0 49.1)
;(find-bundle-throw 7 20 1 9780)         ; ->  (#f #f)
